...
export const MentorMenteeProfile = props => {
  ...
  console.log('fetch data response is, ', firstName, lastName, email, number);
  useEffect(() => {
    FetchAllData();
    setId(getuserData.response.data._id);
    return () => {
      setFirstName('');
      setLastName('');
      setEmail('');
...
console.log("first name of mentor show",firstName)
  const FetchAllData = () => {
    let DataOb = {
      user_id: '63ddf6beecee88802bb66f3a',
    };
    console.log('Fetch function call and ID', id);
    console.log('Data');
    dispatch(
